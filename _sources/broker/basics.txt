...
contains by typing ``header.`` <tab> or using the built-in Python functions
`vars`.
...
One level above the DataBroker sits the DataMuxer, an object for synthesizing
streams of Events from mamy sources into a table. Build a DataMuxer like so:
...
implest task is to simply look at the data from one source -- say, sample
temperature.
...
Therfore, plotting one dependent variable against another usually requires 
...
sub-column, other summary statistics are autmoatically generated.
...
The tabular results from the DataMuxer are DataFrames, and there are many
convenient methods for exporting them to common formats. For example:
...
To quickly look up recent scans, using the standard Python slicing syntax for
